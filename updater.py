...
def update_csv_file(loto_type, filepath):
    """
    メインアプリ（loto-predictor_app.py）の要求仕様に完全適合した、
    (DataFrame, message) を返却するデータ自動更新メイン関数
    """
    if not os.path.exists(filepath):
        return None, f"⚠️ CSVファイルが見つかりません: {filepath}"
        
    # 安全のため、最初に既存のCSVデータを読み込んでキープ（失敗時のフォールバック用）
    try:
        ...
        try:
            ...
            return None, f"⚠️ CSVの読み出しに失敗しました: {e}"
            
    # ハイブリッド型WEBデータチェッカーを起動（創楽URLへ接続）
    web_data = fetch_latest_loto_data_hybrid(loto_type)
    if not web_data or 'round' not in web_data or 'numbers' not in web_data:
        # WEBデータの取得に失敗した場合は、エラーを吐かずに既存のdfをそのまま返してアプリの起動を維持する
        return df, f"⚠️ {loto_type}の最新WEBデータの取得に失敗したため、ローカルの既存履歴データで解析を行います。"
    ...
    if not round_col or not target_cols:
        ...
        return df, "⚠️ CSVの列構造（回号列または本数字列）を特定できませんでした（更新なしで継続）。"
    ...
    try:
        ...
        df.columns = orig_columns
        return df, "⚠️ CSV内の最新回号を解析できませんでした（更新なしで継続）。"
        
    # すでに最新状態かチェック
    if web_data['round'] <= csv_latest_round:
        df.columns = orig_columns
        return df, f"✅ すでに最新データ（第{csv_latest_round}回）に更新されています。"
    ...
    try:
        ...
        return df, f"🎉 【創楽同期】第{web_data['round']}回の最新データをWEBから自動取得し、CSVへ追加しました！"
    except Exception as e:
        return df, f"⚠️ CSVファイルの上書き保存に失敗しました（メモリ上のみ更新）: {e}"
